...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var notificationPanel = document.querySelector("[data-notifications-panel]");
    ...
    if (notificationPanel) {
        var notificationList = notificationPanel.querySelector("[data-notification-list]");
        var feedUrl = notificationPanel.getAttribute("data-notification-feed-url");
        var markReadUrl = notificationPanel.getAttribute("data-notification-mark-read-url");
        var navBadges = document.querySelectorAll(".nav-count");
        ...
        function renderNotifications(items) {
            ...
            if (!items.length) {
                notificationList.innerHTML = '<p class="notification-empty" data-notification-empty>No notifications yet.</p>';
                return;
            }

            notificationList.innerHTML = items.map(function (item) {
                return [
                    '<a href="' + escapeHtml(item.link) + '" class="notification-item' + (item.is_read ? "" : " notification-unread") + '">',
                    "<div>",
                    "<strong>" + escapeHtml(item.message) + "</strong>",
                    "<p>" + escapeHtml(item.created_at) + "</p>",
                    "</div>",
                    '<span class="notification-link">Open</span>',
                    "</a>",
                ].join("");
            }).join("");
        }
        ...
        function clearUnreadState() {
            var unreadItems = notificationPanel.querySelectorAll(".notification-unread");
        ...
        function refreshNotifications() {
            ...
            fetch(feedUrl, {
                ...
                .then(function (data) {
                    ...
                    var unreadCount = (data && data.unread_count) || 0;
                    renderNotifications(items);
                    renderNavBadge(unreadCount);
                    if (unreadCount > 0) {
