...
def customer_issues():
    ...
    cur = mysql.connection.cursor()
    cur.execute(f"SELECT * FROM issues WHERE user_id = %s ORDER BY {sort_expr} {sort_dir_sql}", (session["user_id"],))
    issues = cur.fetchall()
    ...
    return render_template("customer_issues.html", issues=issues, sort_by=sort_by, sort_dir=sort_dir)
